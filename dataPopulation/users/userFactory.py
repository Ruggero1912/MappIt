...
class UserFactory:
    ...
    def create_user_from_scratch(generate_social_relations : bool = None):
        """
        generates and stores in the databases a user object from scratch
        - the user's fields are generated by the User class (which uses Faker)
        :param generate_social_relations if set to True, generates social relations for the created user, else defaults to the value
        of UserFactory.GENERATE_SOCIAL_RELATIONS
    ...
    def close():
        print("Received call to UserFactory.close()! :)")
        UserFactory.neo_driver.close()
